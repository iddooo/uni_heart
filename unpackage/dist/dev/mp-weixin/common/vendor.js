...
module.exports = {
  ...
  // 获取人脸识别头像
  getFaceUrl: function getFaceUrl(data) {
    return fetch({
  ...
  /**公益值*/
  // 公益值明细
  welfareRecord: function welfareRecord(data) {
    return fetch({
  ...
  /**我的二维码*/
  // 查询二维码完善信息
  getCodeInfo: function getCodeInfo(id) {
    return fetch({
...
module.exports = {
  ...
  // 用户设置头像
  uploadHeadPic: function uploadHeadPic(data) {
    var token = uni.getStorageSync('token');
    return new Promise(function (resolve, reject) {
      uni.uploadFile({
  ...
  uploadFace: function uploadFace(data) {
    var token = uni.getStorageSync('token');
    //  上传人脸 / 重新上传
    var url = BASE_URL + '/user-api/user/facePic/' + data.userId;
    if (data.hasPhoto) {
